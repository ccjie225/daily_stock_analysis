...
- [改进] 基金分析页补充参考 PE/PB 估值、同类风险收益、重仓持股、行业配置和费用规则展示。
- [改进] 基金分析页区分基金披露持仓与个人基金持仓，并修复侧栏证据缺口卡片被撑高的问题。
- [新功能] 新增个人基金持仓截图导入预览，可识别支付宝、天天基金、养基宝等场外基金持仓截图并展示待核对字段。
- [改进] 将个人基金持仓截图导入从基金分析页拆分为独立“基金持仓”菜单，避免单基金分析和账户持仓混在同一页面。
- [新功能] 基金持仓页支持将已核对的截图识别结果保存到本地持仓库，并展示已保存持仓列表。
- [新功能] 基金持仓页新增最新公开净值对齐、估算浮动、规则持仓建议和可选 LLM 增强组合复盘。
- [修复] 修正桌面侧边栏在“基金持仓”等较长菜单选中时高亮背景溢出侧栏的问题。
- [改进] 基金持仓复盘支持在缺少基金代码时按基金名称自动匹配公开基金代码，并跳过 A/C 份额歧义匹配。
- [改进] 基金持仓导入预览和已保存持仓支持手动补齐基金代码，并避免补码时生成重复持仓。
- [改进] 基金持仓复盘在缺少份额或成本时可按持有金额、截图净值、收益和收益率自动反推估算字段，并展示估算依据。
