...
识别后可人工核对并保存到本地持仓库，保存接口为 `POST /api/v1/funds/holdings`，列表接口为 `GET /api/v1/funds/holdings`。导入预览和已保存持仓都支持手动补齐 6 位基金代码；若截图只识别到基金名称、没有基金代码，复盘会按基金名称尝试唯一匹配公开基金代码，名称缺失、匹配不到或 A/C 份额存在歧义时不会自动绑定，需人工补齐或修正识别结果。基金持仓页还可调用 `GET /api/v1/funds/holdings/review`，基于已保存份额、成本和最新公开净值估算当前市值、相对截图保存金额的变化、估算盈亏，并给出规则持仓建议；传入 `use_ai=true` 时会在本地规则建议基础上尝试调用已配置 LLM 生成组合复盘，LLM 失败时自动降级到规则建议。场外基金没有股票式逐笔实时价，该能力使用最新公开净值/最近交易日净值做对齐，盘中估算只作为参考。
