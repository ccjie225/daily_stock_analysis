...
After recognition, rows can be manually checked and saved to the local holding store via `POST /api/v1/funds/holdings`; saved rows can be listed with `GET /api/v1/funds/holdings`. Both the import preview and saved holdings list allow manually filling the 6-digit fund code, holding amount, shares, and cost. If a screenshot only provides the fund name without a fund code, review attempts to resolve a unique public fund code by name; missing names, no match, or ambiguous A/C share classes are not auto-bound and should be corrected manually. The holdings page can also call `GET /api/v1/funds/holdings/review` to align saved shares and cost fields with the latest public NAV, estimate current market value, change from the saved screenshot amount, estimated gain, and rule-based holding suggestions. If shares are missing but holding amount is available, the system infers shares by dividing holding amount by latest public NAV. If cost is missing but gain or gain ratio is available, it attempts to infer cost and labels the valuation basis. Passing `use_ai=true` attempts an LLM-enhanced portfolio review on top of the local rules and falls back to rule suggestions if the LLM call fails. Offsite mutual funds do not have stock-like tick-by-tick real-time prices; this feature uses the latest public NAV or most recent trading-day NAV for alignment, while intraday estimates remain reference-only.
