...
class FundHoldingReviewItem(FundSavedHoldingItem):
    ...
    inferred_holding_share: Optional[str] = Field(None, description="按持有金额/收益字段反推的持有份额")
    inferred_cost_amount: Optional[str] = Field(None, description="按成本净值/收益字段反推的持仓成本")
    valuation_basis: List[str] = Field(default_factory=list, description="本次估算使用的持仓字段或反推依据")
