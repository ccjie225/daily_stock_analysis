...
describe('BacktestPage', () => {
  it('renders shared surface inputs and prediction tracking outputs', async () => {
    ...
    const filterInput = await screen.findByPlaceholderText('输入股票代码筛选，留空表示全部');
  ...
  it('switches to next-day validation with the 1D shortcut', async () => {
    ...
    await screen.findByPlaceholderText('输入股票代码筛选，留空表示全部');
    ...
    expect(screen.getByText('是否准确')).toBeInTheDocument();
    expect(screen.getByText('次日验证模式会把 AI 预测与下一交易日收盘表现进行对比。')).toBeInTheDocument();
  });
});
